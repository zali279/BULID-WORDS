...
const wordHintSearch =async (hintSearch) => {
    console.log("hintsearch" , hintSearch )
    allLetter=""
    ...
    console.log("hintsearch" , hintSearch )
    console.log("response.data",response.data[hintSearch])
    console.log(response.data[hintSearch])
    console.log(response.data[0])
    hintWord = response.data[hintSearch].word
    console.log("Hint Search" ,hintWord)
    doubleCheakHint()
}
...
hintButton.addEventListener('click',wordHintSearch)
...
let letterArry = []
let englishLetterArray =['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'n', 'o', 'q', 'r', 's', 't', 'u', 'v',  'y']
...
let totaltargettWord =0//for resualt 

let repateLetter=0
let z=4

let foundFinalHint=false
let hintWordArray=[]
...
const randomLetter = () => {
    ...
    for (let i =0 ; i<letterArry.length;i++){
        let letterDiv =document.getElementById(i)
        letterDiv.innerText = letterArry[i]
    }
}
...
const wordHintSearch =async (wordlength) => {
    hintWordArray=[]
    let allLetter=""
    letterArry.forEach((element) => {
        allLetter +=element
    ...
    let hintLink=`https://api.datamuse.com/words?sp=[${allLetter}]{${wordlength}}&max`
    let response = await axios.get (hintLink)
    
    console.log("respons",response.data[0].word)
    console.log("respons",response.data[2].word)
    //console.log("lenght",response.data.length)
    
    for (let i=0;i<response.data.length;i++){
        hintWordArray.push(response.data[i].word)
        //console.log(hintWordArray)
    }
    //console.log("hint word array ",hintWordArray)
    /*
    hintWord = response.data[hintSearch].word
    ...
    */
    ...
    for (let i =0 ; i< hintWordArray.length ; i++){
    if (foundFinalHint===true){
        break;
    }
    else {
        await doubleCheakHint(hintWordArray[i])
    }
    }
...
const doubleCheakHint = async (hintWordcheck) => {
    //await hintWordArray.forEach((element) => {    
    wordLink = `${englishWordsLink}${hintWordcheck}`
    //wordLink = `${englishWordsLink}${element}`
    console.log("hint word check:",hintWordcheck)
        await fetch(wordLink)
                .then(response => {
                    if (!response.ok) {
                        //wordHintSearch(z)
